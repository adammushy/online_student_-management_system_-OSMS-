...
def generate_pdf(request, *args, **kwargs):
    ...
    context = {"student_list": mystudentlist}  
    print(context)
    ...
    # pdf = canvas.Canvas(buffer, pagesize=landscape(letter), bottomup=0)
    
  
    # # convert HTML to PDF using weasyprint
    # HTML(string=html).write_pdf(pdf)
    
    # # close the PDF canvas and write to the buffer
    # pdf.save()
...
def add_score_for(request, id):
    ...
    if request.method == 'POST':
        ...
        for s in range(0,len(ids)):      # iterate over the list of student ids gathered above
            ...
            obj.comment = obj.get_comment(obj.grade,ca,exam)
